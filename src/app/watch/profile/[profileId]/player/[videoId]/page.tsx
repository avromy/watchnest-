import { BackLink, ErrorState, PageHeader } from "@/components/ui-states";
import {
  getApprovedVideosForProfile,
  isProfileAuthorizedToPlayVideo,
} from "@/lib/server/watchnest-data";
...
function PlaybackUnavailable({
  ...
  return (
    <main className="min-h-screen bg-watchnest-background px-6 py-10 text-watchnest-foreground">
      <ErrorState
        action={<BackLink href={backToLibraryHref}>Back to Library</BackLink>}
...
export default async function ViewerPlayerPage({
  params,
}: {
  params: { profileId: string; videoId: string };
}) {
  const backToLibraryHref = `/watch/profile/${params.profileId}/home`;
  const isAuthorized = await isProfileAuthorizedToPlayVideo(
    params.profileId,
    params.videoId,
  );
    ...
    <main className="min-h-screen bg-watchnest-background px-6 py-8 text-watchnest-foreground">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-6">
          <PageHeader
            description="This player is limited to the current profile's approved library."
            eyebrow="Approved video"
            title={video.title}
          />
